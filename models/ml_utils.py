...
def abs_to_lab(data):
    ...
    file_name = pd.concat(
        [
            pd.DataFrame(
                ast.literal_eval(electrochromics.wavelength.values[0]),
                columns=['wavelength'],
            ),
            scaled_df,
        ],
...
def tecan_read(filename):
    ...
    file_name = pd.read_csv(
        'polybot_app/demo_files/from_tecan/green_loop_3.csv'
    )  # (csv_filepath)
    print('file_name', file_name)
    scaled_data = scaler.fit_transform(file_name.iloc[:, 1:])
    ...
    file_name.to_csv(
        'polybot_app/demo_files/from_tecan/green_loop_3_scaled.csv', index=None
    )
    # return csv_filepath
