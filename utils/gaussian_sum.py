...
def gauss_sum_model(x, params):
    return np.sum(
        [
            gauss(x, params[i], params[i + 1], params[i + 2])
            for i in range(0, len(params), 3)
        ],
        axis=0,
    )
...
folder_name = r"C:/Users/kvriz/Desktop/Polybot_ECPs\datasets/absorption_spectra/literature_data/ACS Macro Lett., 5, 714-717, 2016"
...
initial_means = np.linspace(
    data['Wavelength (nm)'].min() + peak_spacing,
    data['Wavelength (nm)'].max() - peak_spacing,
    num_peaks,
)
...
initial_params = np.ravel(
    list(zip(initial_heights, initial_means, initial_widths))
)
...
initial_centers = np.linspace(
    data['Wavelength (nm)'].min() + 2.5,
    data['Wavelength (nm)'].max() - 2.5,
    num_peaks,
)
...
# Flatten the initial parameters into a single array (for the optimizer)
initial_params = np.ravel(
    list(zip(initial_amplitudes, initial_centers, initial_widths))
)
...
result_with_bounds = least_squares(
    objective_function,
    initial_params,
    args=(data['Wavelength (nm)'], data['Absorbance']),
...
if result_with_bounds.success:
    ...
    plt.plot(
        data['Wavelength (nm)'],
        gauss_sum_model(data['Wavelength (nm)'], optimized_params_with_bounds),
        label='Fitted 20 Peaks with Bounds',
        color='red',
        zorder=2,
    )
    plt.xlabel('Energy (eV)')
