...
params, _ = curve_fit(
    gauss, data['Wavelength (nm)'], data['Absorbance'], p0=initial_params
)
...
reconstructed_spectrum = np.sum(
    [
        gauss(data['Wavelength (nm)'], *params[i : i + 3])
        for i in range(0, len(params), 3)
    ],
    axis=0,
)
...
plt.plot(
    data['Wavelength (nm)'], data['Absorbance'], label='Original Spectrum'
)
plt.plot(
    data['Wavelength (nm)'],
...
plt.plot(
    data['Wavelength (nm)'], fitted_peaks, label='Fitted Peaks', linestyle='-.'
)
for i in range(0, len(params), 3):
    plt.fill_between(
        data['Wavelength (nm)'],
