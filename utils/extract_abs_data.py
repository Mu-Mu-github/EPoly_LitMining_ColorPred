...
dataset = pd.concat(
    [
        pd.DataFrame(oleic_oleyl, columns=['oleic_oleyl']),
        pd.DataFrame(oleic_pb, columns=['oleic_pb']),
        pd.DataFrame(Cs_Pb, columns=['Cs_Pb']),
        pd.DataFrame(peak_wavelengths),
        pd.DataFrame(peak_intensity),
        pd.DataFrame(hwhm),
    ],
    axis=1,
)
