...
def tecan_read(filename):
    ...
    if os.path.exists(filename):
        ...
        csv_filepath = filename.replace(
            os.path.basename(filename), csv_filename
        )
    ...
    file_name.to_csv(
        'polybot_app/demo_files/from_tecan/Loop-3-20240402-normalized_scaled.csv',
        index=None,
    )
    return csv_filepath
...
def smile_to_dft(smile):
    dictionary, _ = get_dft_descriptors_dictionary(
        'polybot_app/ml_loop/dft_descriptors_ECPs.csv'
    )
...
def create_predictions_dataset_dft(inventory, num_monomers, ratio_step):
    ...
    if 2 in num_monomers:
        two_monomer_combinations = [
            [first_monomer, mol, 0] for mol in rest_monomers
        ]
    ...
    if 3 in num_monomers:
        three_monomer_combinations = [
            [first_monomer] + list(mol_pair)
            for mol_pair in combinations(rest_monomers, 2)
        ]
        for ratio in range(100, 49, -ratio_step):
    ...
    dataset[["smiles1", "smiles2", "smiles3"]] = dataset["Combination"].apply(
        pd.Series
    )
    dataset[["percentage_1", "percentage_2", "percentage_3"]] = dataset[
        "Ratio"
    ].apply(pd.Series)
...
def get_train_data_representation(dataframe):
    ...
    df3 = bits_to_df(
        dataframe.smiles3, 'bit_2'
    )  # (np.repeat(0, dataframe.shape[0],axis=0), 'bit_3')
    ...
    dataset = pd.concat(
        [
            df1,
            pd.DataFrame(percentage_1.values, columns=['percentage_1']),
            df2,
            pd.DataFrame(percentage_2.values, columns=['percentage_2']),
            df3,
            pd.DataFrame(percentage_3, columns=['percentage_3']),
        ],
        axis=1,
    )
    return dataset
...
def get_train_data_representation_dft(dataframe):
    ...
    df2 = bits_to_df(dataframe.smiles2, 'bit_2')
    df3 = bits_to_df(
        dataframe.smiles3, 'bit_3'
    )  # (np.repeat(0, dataframe.shape[0],axis=0), 'bit_3')
    df1_dft = dft_descr_from_df(
        np.repeat(smiles_A, dataframe.shape[0], axis=0), 'bit_1'
    )
...
def select_next_exp_ml(dataframe, selected_color, uncertainties, iteration):
    ...
    selected_color_lab = LabColor(
        selected_color[0], selected_color[1], selected_color[2]
    )
    print('selected_color_lab', selected_color_lab)
    # print('dataframe',dataframe)
    dataframe['delta_e'] = dataframe.apply(
        lambda row: delta_e_cie2000(
            selected_color_lab, LabColor(row['L'], row['a'], row['b'])
        ),
        axis=1,
    )

    if iteration == 0 or iteration == 1:
        EI_values = expectedImprovement(
            dataframe['delta_e'].values, uncertainties, ybest=0, epsilon=1
        )
        # upperConfidenceBound(dataframe['delta_e'].values, uncertainties, epsilon=0.01)
    else:
        EI_values = expectedImprovement(
            dataframe['delta_e'].values, uncertainties, ybest=0, epsilon=0.1
        )
    ...
    plt.scatter(
        top_6_loc,
        dataframe.loc[top_6_loc, :]['uncertainties'],
        color='r',
        marker='s',
        s=100,
        facecolor='None',
    ...
    plt.scatter(
        top_6_loc,
        dataframe.loc[top_6_loc, :]['EI_values'],
        color='r',
        marker='s',
        s=100,
        facecolor='None',
...
def expectedImprovement(ye_pred, esigma, ybest, epsilon=0.01):
    ...
    for ii in range(ye_pred.size):
        if esigma[ii] > 0:
            ...
            expI[ii] = (ye_pred[ii] - ybest - epsilon) * norm.cdf(
                zzval
            ) + esigma[ii] * norm.pdf(zzval)
        else:
            expI[ii] = 0.0
    return expI
...
def convert_for_chemspeed(dataframe):
    ...
    csv = pd.DataFrame(
        ...
        columns=["ProDOT-2Br", "Ben-2Br", "BTD-2Br", "DMoT-2Br", "EDOT-2Br"],
    )
...
def peak_check(df):
    ...
    for col in abs_spectra.columns[1:]:
        ...
        if len(peaks) > 0:
            ...
            highest_peak_absorbance = properties['peak_heights'][
                highest_peak_idx
            ]
