...
def align_abs_spectra(file_path):
    ...
    sd_copy.interpolate(
        colour.SpectralShape(low_wavelength, high_wavelength, 1)
    )
    ...
    for root, directories, files in os.walk(parent_directory_path):
        for filename in files:
            ...
            if filename.endswith(".csv"):
                ...
                scaled_df = pd.DataFrame(scaled_data, columns=['Absorbance'])
                file_name = pd.concat(
                    [data_frame['Wavelength (nm)'], scaled_df], axis=1
                )
                # plt.plot(file_name["Wavelength (nm)"], file_name["Absorbance"])
                # plt.show()
                file_name.to_csv(
                    f"{root}/{filename.rsplit('.', 1)[0]}_aligned.csv",
                    index=None,
                )
        # except:
        #     print(root , filename)
...
def get_all_abs_from_files(parent_directory_path):
    ...
    for root, directories, files in os.walk(parent_directory_path):
        for filename in files:
            if filename.endswith("_aligned.csv"):
                ...
                wavelength_vector.append(
                    [", ".join(map(str, df['Wavelength (nm)'].values))]
                )
                intensity_vector.append(
                    [", ".join(map(str, df['Absorbance'].values))]
                )
    ...
    dataset = pd.concat(
        [
            pd.DataFrame(source, columns=['source']),
            pd.DataFrame(polymer_name, columns=['polymer_id']),
            pd.DataFrame(wavelength_vector, columns=['wavelength']),
            pd.DataFrame(intensity_vector, columns=['intensity']),
        ],
        axis=1,
    )
...
parent_directory_path = (
    'C:/Users/kvriz/Desktop/Polybot_ECPs/datasets/absorption_spectra/'
)
