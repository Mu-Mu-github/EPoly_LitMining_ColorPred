...
def weighted_mse_loss(input, target):
    ...
    )  # .cuda()  # Higher weight for the a and b values than for the L
...
class CoPolymerSetTransformer(nn.Module):
    def __init__(
        ...
        self.use_abs_decoder = (
            use_abs_decoder  # Setting the flag as a class attribute
        )
    ...
    def forward(self, x):
        ...
        if self.use_abs_decoder:
            ...
            return output1.squeeze(-1).squeeze(1), output2.squeeze(-1).squeeze(
                1
            )
    ...
    def train_model(self, data1, data2, data3, y_lab=None, y_abs=None):
        ...
        optimizer1 = torch.optim.Adam(
            self.dec1.parameters(), lr=self.learning_rate
        )
        ...
        if self.use_abs_decoder:
            optimizer2 = torch.optim.Adam(
                self.dec2.parameters(), lr=self.learning_rate
            )
        ...
        for epoch in tqdm.tqdm(range(self.epochs)):
            ...
            for x, y in DataLoader(
                MonomerPair(data1, data2, data3, y_lab),
            ...
            if self.use_abs_decoder:
                for x, y in DataLoader(
                    MonomerPair(data1, data2, data3, y_abs),
                    batch_size=self.batch_size,
                ):
                    x, y = x.to(self.device).float(), y.to(self.device).float()
    ...
    def test_model(self, data1, data2, data3, target_lab, target_abs=None):
        ...
        for i in range(10):
            ...
            for x, y in DataLoader(
                MonomerPair(data1, data2, data3, target_lab),
            ...
            if self.use_abs_decoder:
                for x, y in DataLoader(
                    MonomerPair(data1, data2, data3, target_abs),
                    batch_size=len(data1),
                ):
                    x, y = x.float(), y.float()  # .to(self.device)
                    y = self(x)
                ...
                return (
                    np.mean(abs_list, axis=0),
                    np.mean(lab_list, axis=0),
                    overal_std,
                )

        return np.mean(lab_list, axis=0), np.std(
            np.std(y_list_std1, axis=0), axis=1
        )
