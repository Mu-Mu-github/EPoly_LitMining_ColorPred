...
def get_smoothed_intensity(dataset, window_size=30, poly_order=3):
    ...
    for i in range(dataset.shape[0]):
        y = apply_savgol_filter(
            ast.literal_eval(dataset.intensity[i]), window_size, poly_order
        )
        df_y.append(y)
    return df_y
...
def main():
    ...
    parser.add_argument(
        '--training_data', help='The csv file with the training smiles pairs'
    )
    parser.add_argument('--dropout_ratio', help='Set the dropout_ratio')
    parser.add_argument(
        '--save_dir', help='The directory to save the trained network'
    )
    parser.add_argument('--model_name', help='Name of the trained network')
    parser.add_argument(
        '-n_epochs', default=50, type=int, help='Set the number of epochs'
    )
    parser.add_argument(
        '-lr', default=0.001, type=float, help='Set the number of epochs'
    )
    parser.add_argument(
        '-batch_size', default=32, type=int, help='Set the batch size'
    )
    parser.add_argument(
        '--use_abs_decoder',
        action='store_true',
        help='Use the absorption decoder',
    )
    args = parser.parse_args()

    # Load the training data
    electrochromics = pd.read_csv(args.training_data)
    electrochromics['smiles1'] = electrochromics['smiles1'].str.replace(
        '*', 'C'
    )
    electrochromics['smiles2'] = electrochromics['smiles2'].str.replace(
        '*', 'C'
    )
    electrochromics['smiles3'] = electrochromics['smiles3'].str.replace(
        '*', 'C'
    )
    df = electrochromics[
        [
    ...
    train_data_1, train_data_2, train_data_3 = (
        np.array(training_dataset.iloc[:, :Ndims].values, dtype=float),
        np.array(
            training_dataset.iloc[:, Ndims : 2 * Ndims].values, dtype=float
        ),
        np.array(training_dataset.iloc[:, 2 * Ndims :].values, dtype=float),
    )
    ...
    losses1, losses2 = model.train_model(
        train_data_1, train_data_2, train_data_3, y_lab, y_abs
    )
    ...
    torch.save(
        ...
        os.path.join(
            args.save_dir, 'co_polymer_set_transformer_new_model.tar'
        ),
    )
